...
sub write_output {
    ...
    foreach $1 (@output) {
        ...
        # if an OR construction is found, initialize extra identation
        if ($1 =~ m/OR\(/ eq "1") {
            $firstbracket = 1 + index $1, "(";
            $lastbracket = rindex $1, ")";
            $size        = $lastbracket - $firstbracket;
            $ornumber    = substr $1, $firstbracket, $size;
            $oramount    = 0;
            $ident       = 4;
        }

        # reset OR identation once the amount of conditions is reached
        if ($ornumber > 0 && $oramount eq $ornumber) {
            $ident = 2;
            $ornumber = 0;
        }

        if ($1 eq "THEN" || $1 eq "IF") {
            $ident = 2;
        }

        if ($1 =~ m/RESPONSE/ eq "1") {
            $ident = 4;
        }
    }
...
sub get_combine_top {
    ...
    foreach $forcombine (@action) {
        ...
        {
            die
"Error near line $linenum: $actionargs[0] is not valid at present in Combine() ";
        }
        else {
            die
"Error near line $linenum: $actionargs[0] is not a recognised action type";
        }
...
sub block_print {
    ...
    foreach $3 (@target) {
        ...
        if (    $targetcondition[$conditioncounter] ne "None"
            and $targetcondition[$conditioncounter] ne "" )
        {
